...
- Required behavior:
  ...
  - Entity-list columns use bounded content sizing, preserve manually adjusted widths during filtering, and sort text with numeric segments naturally. Column filters track horizontal scrolling; the property and I/O filter panel can be collapsed, and window size is remembered for the application session.
  ...
  - EntityList can add its selected rows to an independent fixed-highlight collection, remove selected rows from it, or clear it; pinned rows keep a tinted background and left-edge marker without an extra column, including after filtering; closing and reopening the list clears that collection while ordinary selection clearing on entering Walk mode (`X`) remains unchanged.
  - All list actions remain below the grid: a pinned-highlight count with Add/Remove/Clear, a separate filter/column-width/MAP-to-list group, and an independent Export button. Groups wrap together in narrow windows; tooltips explain synchronization direction and that clearing pinned highlights keeps ordinary selection.
  - `Ctrl+G` navigates by entity target name, world coordinate, or Hammer ID. Target-name lookup tries an exact match first, then a case-insensitive partial match, and prompts when multiple nodes match. The resizable multi-match dialog shows entity name, classname, and Hammer ID, with column-header toggles for ascending or descending natural sorting. Like the entity list, it preserves input order until a column header is clicked.
  - `Ctrl+Shift+E` moves the camera to the currently selected entity using the EntityList double-click focus behavior; with multiple selected entities, it prompts for one entity using the GoTo-style selection dialog.
  - `Alt+Enter` opens information for the selected entity or scene node.
  - The standalone EntityInfo window leaves single-click property interactions passive; double-clicking a property value tries coordinate, Hammer ID, and entity-name navigation, selecting and focusing matched entities and reusing the multiple-match dialog. Resource-reference values retain their external-open behavior.
...
- Invariants: navigation must search both the main and 3D skybox scenes, MAP-to-list synchronization defaults off and is not persisted, list-to-MAP selection remains active regardless of that checkbox, fixed highlights remain independent of ordinary selection and do not enter MAP/list synchronization, deleting coordinate markers removes their fixed highlights, pinned-only light-probe debug grids are cleared when unpinned or leaving the debug mode, synchronization must not create an event loop, coordinate navigation must preserve camera-transition history, and opening a second entity-list window for the same viewer must be avoided.

### Context Menu Extensions

- Purpose: keep Custom context-menu entry points together while reusing existing opening, export, and viewer workflows.
- Required behavior:
  - Explorer VPK file context menus and the opened VPK context menu offer "Find and open map", recursively searching the selected package and nested VPK entries for `vmap_c` maps. A single match opens directly; multiple matches prompt once in a two-column map/package list with nested package paths and full-path tooltips. No matches show an informational message, cancellation opens nothing, and recursive traversal avoids duplicate packages or cycles. This uses the ordinary resource-opening pipeline.
  - Map and world entries in the VPK context menu offer "Open without entity simulation". This per-open option disables the entity system before world loading and the first simulation tick, including the shared skybox entity system. The World panel checkbox can enable simulation afterwards; ordinary opens and reloads retain the enabled default, and no preference is persisted.
  - Explorer context menus offer "Refresh" to discover newly installed game and Workshop content. It clears cached game and Workshop entries and reruns the existing game-folder scan. Only one scan may run at a time; refresh requests during an active scan are ignored, and existing scanning status text and behavior remain unchanged.
  - Supported VPK file and open-tab context menus offer "Export entities" through the shared pipeline described in Entity JSON Export.
  - The entity I/O graph tab alone offers "Open in New Window" with the ownership and rendering behavior described in Independent Entity I/O Graph Window and Auxiliary Windows.
- Main integration points: `GUI/MainForm.MapOpen.cs`, `GUI/Controls/ExplorerControl.MapOpen.cs`, `GUI/Forms/MapSelectionForm.cs`, `GUI/MainForm.EntitySimulation.cs`, `GUI/MainForm.ContextMenus.cs`, `GUI/Controls/ExplorerControl.cs`, `GUI/Controls/ExplorerControl.Designer.cs`, `GUI/Controls/TabWindowMenu.cs`, `GUI/Utils/VrfGuiContext.cs`, and `GUI/Types/GLViewers/GLWorldViewer.cs`.
...
- Required behavior:
  ...
  - The context-menu entry described in Context Menu Extensions reuses one independent window per world viewer.
...
- Required behavior:
  - Entity export is available from the context-menu entries described in Context Menu Extensions and from the world viewer entity list.
  ...
  - Property-filtered JSON preserves native KV value types for selected properties and emits each available connection's output, target, input, parameter, delay, fire count, and target type.
...
- The CLI `--export_entities` mode reuses the library extraction and serializer through `CLI/Decompiler.EntityExport.cs`; it accepts one Map/World resource and must not change GUI selection or export behavior.
...
- Required behavior:
  ...
  - Fixed-highlighted entities with a target name display that name above the existing node or classname label, with spacing adapted to the projected text height; ordinary selection and unpinned entities use the existing label only.
  - World-viewer Physics Groups without a leading hyphen are sorted by name, while leading-hyphen groups retain their existing order and `S2V: Render as opaque` remains fixed at the end.
...
- Invariants: search covers static and dynamic nodes, selection highlighting remains visible when initiated from the entity list, renderer selection reads use stable snapshots while UI tools may update selection concurrently, bulk selection and fixed-highlight changes are committed as one render-visible batch without intermediate per-node states, highlight-style changes remain local to the current viewer and default rendering stays unchanged, the bulk-selection predicate does not affect manual item selection, and the default collision-group marker is only assigned to the world default group rather than entity-owned physics.
...
### Gameplay Flow Recording

- Purpose: record, narrate, persist, and revisit focused gameplay scenes while exploring a map.
- Required behavior:
  - The world-viewer Tools controls open one reusable gameplay-flow window per world viewer.
  - A document normally belongs to one map and can contain multiple independent root flows. Any node can optionally own one child flow, allowing finite nested flows without changing the default flat workflow.
  - Each flow stores nodes in reading order with automatic hierarchical numbering, optional non-contiguous groups that can form an acyclic hierarchy, and explicit acyclic connections that support branches, merges, and disconnected paths.
  - Capturing creates a node without blocking and records the camera position, QAngle, FOV, renderable entity selection, node-owned annotation points, and viewer-shortcut visibility mask. Selecting a node restores that focused state immediately without changing other viewer visibility or rendering preferences.
  - Routine flow, node, group, and child-flow creation and deletion are immediate; new containers receive unique default names, while unsaved-change confirmation is deferred until closing or replacing the document.
  - Complete documents can be opened and saved as formatted JSON or copied to and replaced from clipboard JSON through one shared validation pipeline.
  - Persisted JSON uses explicit stable property names and checks its format version before binding to the current document model, providing one upgrade boundary for future schema migrations.
- Main integration points: `GUI/Forms/GameplayFlowForm.cs`, `GUI/Types/GameplayFlow/`, `GUI/Types/GLViewers/GLWorldViewer.cs`, `GUI/Utils/KeybindingRegistry.cs`, `Renderer/Renderer/Scene.cs`, and `Renderer/Renderer/ViewerVisibilityState.cs`.
- Invariants: node UUIDs provide identity while array order alone determines numbering; group nesting only organizes the timeline and cannot form cycles; empty groups remain visible and manageable; deleting a group promotes its direct nodes and child groups to its parent; `hammeruniqueid` is resolved only inside the recorded map and scene; child flows have one owner and cannot recursively reference an ancestor; node graph connections cannot form cycles; annotations remain session-only and never enter loaded-world storage, entity I/O resolution, or entity export; file and clipboard import check and upgrade the format before validating a temporary document for replacement; newer unsupported formats are rejected without binding or replacement; clipboard export does not clear dirty state and clipboard import requires Save As; restoration preserves camera-transition history and does not change World Layers, Physics Groups, mesh groups, render mode, exposure, or selection-highlight settings; closing the owning viewer removes annotation GL resources before renderer disposal.
...
2. Manually open a map in the GUI and verify the four shortcuts, including selected-entity camera focus, exact and partial navigation, multiple-match selection, entity filtering, row selection/highlighting, and camera navigation.
3. In entity info, verify Hammer ID display and sorting, property-value double-click navigation, then follow one connection Output-to-Input and Input-to-Output. Include a special or multi-target connection when test data provides one. In the I/O graph, verify Ctrl-click, Ctrl+A, merged-node expansion, MAP highlight synchronization without camera movement, and unchanged Ctrl-drag behavior.
