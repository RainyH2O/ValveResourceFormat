...
#pragma warning restore CA2000
        ...
        bool includeConnectionRelated)
    {
        ...
        var processed = new HashSet<EntityLump.Entity>(ReferenceEqualityComparer.Instance);
        ...
        Queue<EntityLump.Entity> queue)
    {
        foreach (var (key, value) in entity.Children)
        {
            ...
            if (string.IsNullOrEmpty(referencedName) ||
                ...
                referencedName.Contains(' ', StringComparison.Ordinal) ||
                referencedName.Contains(',', StringComparison.Ordinal) ||
                referencedName.Contains(';', StringComparison.Ordinal))
            {
                continue;
        ...
        HashSet<string> selectedProperties)
    {
        ...
        var filteredEntities = entities.Select(entity =>
        {
            ...
            return EntityExportSerializer.ToLegacyEntity(entity, relevantProperties);
        });

        return KVJsonSerializer.Serialize(ValveKeyValue.KVObject.Array(filteredEntities));
    }
}
