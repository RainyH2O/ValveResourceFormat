...
public partial class EntityListForm : ThemedForm
{
    ...
    private HashSet<EntityLump.Entity> _pinnedEntities = [];
    ...
    public event EventHandler<IReadOnlyList<EntityLump.Entity>>? OnEntitiesPinned;
    public event EventHandler<IReadOnlyList<EntityLump.Entity>>? OnEntitiesUnpinned;
    public event EventHandler? OnPinnedEntitiesCleared;
...
#pragma warning disable CA2000 // Controls are transferred to their parent container's Controls collection and disposed with it
    public EntityListForm(List<EntityLump.Entity>? entities, string defaultExportFileName, string exportPath, Predicate<EntityLump.Entity>? canExportEntity = null)
    {
        ...
        entityDataGridView.SizeChanged += OnGridSizeChanged;
        entityDataGridView.Scroll += OnGridScroll;
        entityDataGridView.ColumnHeaderMouseClick += OnColumnHeaderMouseClick;
        entityDataGridView.RowPrePaint += OnGridRowPrePaint;
        entityDataGridView.RowPostPaint += OnGridRowPostPaint;
        ...
        // ── Export button bar ──────────────────────────────────────────────
        var exportButton = new ThemedButton { Text = "Export", AutoSize = true };
        ...
        var pinButton = new ThemedButton { Text = "Add", AutoSize = true };
        pinButton.Click += (_, _) => OnEntitiesPinned?.Invoke(this, GetSelectedEntities());
        var unpinButton = new ThemedButton { Text = "Remove", AutoSize = true };
        unpinButton.Click += (_, _) => OnEntitiesUnpinned?.Invoke(this, GetSelectedEntities());
        var clearPinnedButton = new ThemedButton { Text = "Clear", AutoSize = true };
        clearPinnedButton.Click += (_, _) => OnPinnedEntitiesCleared?.Invoke(this, EventArgs.Empty);
        var filtersButton = new ThemedButton { Text = "Hide filters", AutoSize = true };
        filtersButton.Click += (_, _) =>
        {
            filtersGroupBox.Visible = !filtersGroupBox.Visible;
            UpdateFiltersButton();
        };
        void UpdateFiltersButton()
        {
            var count = new[] { keyTextBox, valueTextBox, outputTextBox, targetTextBox, inputTextBox }
                .Count(textBox => !string.IsNullOrWhiteSpace(textBox.Text));
            filtersButton.Text = filtersGroupBox.Visible ? "Hide filters" : $"Filters ({count})";
        }
        foreach (var textBox in new[] { keyTextBox, valueTextBox, outputTextBox, targetTextBox, inputTextBox })
        {
            textBox.TextChanged += (_, _) => UpdateFiltersButton();
        }
        var resetColumnsButton = new ThemedButton { Text = "Reset widths", AutoSize = true };
        resetColumnsButton.Click += (_, _) =>
        {
            ManualColumnWidths.Clear();
            PreferredColumnWidths.Clear();
            AdjustColumnWidths();
        };
        var buttonPanel = CreateActionBar(pinButton, unpinButton, clearPinnedButton, filtersButton, resetColumnsButton, exportButton);
...
#pragma warning restore CA2000
    ...
    private void InitDataTable(List<EntityLump.Entity>? entities)
    {
        foreach (var (columnName, _) in _columnsToDisplay)
        {
            entityDataGridView.Columns.Add(new DataGridViewTextBoxColumn
            {
                ...
                MinimumWidth = LogicalToDeviceUnits(70),
    ...
    {
        var filters = _filterTextBoxes.ToDictionary(item => item.Key, item => item.Value.Text);
        _filterTextBoxes.Clear();
        _filterRowPanel = new Panel
        {
            Dock = DockStyle.Top,
        ...
        foreach (DataGridViewColumn column in entityDataGridView.Columns)
        {
            ...
            {
                Tag = column.Name,
                Text = filters.GetValueOrDefault(column.Name, string.Empty),
            };
            textBox.TextChanged += OnColumnFilterTextChanged;
            _filterRowPanel.Controls.Add(textBox);
            _filterTextBoxes[column.Name] = textBox;
        }
    ...
    private void OnGridColumnWidthChanged(object? sender, DataGridViewColumnEventArgs e)
    {
        ...
        {
            ManualColumnWidths[e.Column.Name] = e.Column.Width;
        }
        AlignFilterRow();
    }
    ...
    private void BindData(List<EntityLump.Entity>? entities)
    {
        ...
        _dataTable.Columns.Add(SortOrderColumn, typeof(int));
    ...
    private static string EscapeFilterValue(string filterValue)
    {
        const string lb = "~~LB~~";
        const string rb = "~~RB~~";
        filterValue = filterValue.Replace("[", lb, StringComparison.Ordinal).Replace("]", rb, StringComparison.Ordinal)
    ...
    private void OnGridKeyDown(object? sender, KeyEventArgs e)
    {
        ...
        var allRowsSelected = entityDataGridView.Rows.Count > 0
            && entityDataGridView.SelectedRows.Count == entityDataGridView.Rows.Count;
        ...
        OnEntitySelectionChanged?.Invoke(this, GetSelectedEntities());
        e.Handled = true;
    ...
    {
        if (synchronizingSelection || suppressSelectionChanged)
        {
            return;
        }
    ...
    private EntityLump.Entity[] GetSelectedEntities()
    {
        return entityDataGridView.SelectedRows
    ...
    internal void SetPinnedEntities(IEnumerable<EntityLump.Entity> entities)
    {
        _pinnedEntities = entities.ToHashSet();
        ApplyPinnedRowStyles();
    }
    ...
    private void UpdateTableColumns(string keyFilter, string valueFilter, string outputFilter, string targetFilter, string inputFilter)
    {
        ...
        if (entityDataGridView.Columns.Cast<DataGridViewColumn>().Select(column => column.Name)
            .SequenceEqual(_columnsToDisplay.Select(column => column.ColumnName)))
        {
            return;
        }
        ...
        var previousFilterRow = _filterRowPanel;
        AddFilterRowControls();
        previousFilterRow?.Dispose();
    }
    ...
    private void ApplyPinnedRowStyles()
    {
        entityDataGridView.Invalidate();
    }

    private static Color GetPinnedRowBackColor()
    {
        var baseColor = Themer.CurrentThemeColors.AppMiddle;
        var accent = Color.Gold;
        const int blend = 48;

        return Color.FromArgb(
            baseColor.R + (accent.R - baseColor.R) * blend / 255,
            baseColor.G + (accent.G - baseColor.G) * blend / 255,
            baseColor.B + (accent.B - baseColor.B) * blend / 255);
    }

    private void OnGridRowPrePaint(object? sender, DataGridViewRowPrePaintEventArgs e)
    {
        if (GetEntityAtRow(e.RowIndex) is { } entity && _pinnedEntities.Contains(entity))
        {
            e.InheritedRowStyle.BackColor = GetPinnedRowBackColor();
        }
    }

    private void OnGridRowPostPaint(object? sender, DataGridViewRowPostPaintEventArgs e)
    {
        if (GetEntityAtRow(e.RowIndex) is not { } entity || !_pinnedEntities.Contains(entity))
        {
            return;
        }

        using var brush = new SolidBrush(Color.Gold);
        e.Graphics.FillRectangle(brush, e.RowBounds.Left, e.RowBounds.Top, LogicalToDeviceUnits(4), e.RowBounds.Height);
    }
